...
from ssm_customized.regression import fit_linear_regression, \
    generalized_newton_studentst_dof
...
class Observations(object):
    ...
    # M = exogenous input dimensions (the inputs modulate the probability of
    # discrete state transitions via a multiclass logistic regression)
    ...
    def m_step(self, expectations, datas, inputs, masks, tags,
        ...
        """
        If M-step cannot be done in closed form for the observations,
        default to SGD.
        """
        optimizer = \
        dict(adam=adam, bfgs=bfgs, lbfgs=lbfgs, rmsprop=rmsprop, sgd=sgd)[
            optimizer]
    ...
    def neg_hessian_expected_log_dynamics_prob(self, Ez, data, input, mask,
                                               tag=None):
        raise NotImplementedError
...
class GaussianObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        if mask is not None and np.any(~mask) and not isinstance(mus,
                                                                 np.ndarray):
            raise Exception(
                "Current implementation of multivariate_normal_logpdf for "
                "masked data"
                "does not work with autograd because it writes to an array. "
                "Use DiagonalGaussian instead if you need to support missing "
                "data.")

        # stats.multivariate_normal_logpdf supports broadcasting, but we get
        # significant performance benefit if we call it with (TxD), (D,),
        # and (D,D)
        # arrays as inputs
        return np.column_stack(
            [stats.multivariate_normal_logpdf(data, mu, Sigma)
             for mu, Sigma in zip(mus, Sigmas)])

    def sample_x(self, z, xhist, input=None, tag=None, with_noise=True):
        D, mus = self.D, self.mus
        sqrt_Sigmas = self._sqrt_Sigmas if with_noise else np.zeros(
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        for (Ez, _, _), y in zip(expectations, datas):
            ...
            sqerr += np.sum(
                Ez[:, :, None, None] * resid[:, :, None, :] * resid[:, :, :,
                                                              None], axis=0)
            weight += np.sum(Ez, axis=0)
        self._sqrt_Sigmas = np.linalg.cholesky(
            sqerr / weight[:, None, None] + 1e-8 * np.eye(self.D))
...
class ExponentialObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        return stats.exponential_logpdf(data[:, None, :], lambdas,
                                        mask=mask[:, None, :])
...
class DiagonalGaussianObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.diagonal_gaussian_logpdf(data[:, None, :], mus, sigmas,
                                              mask=mask[:, None, :])

    def sample_x(self, z, xhist, input=None, tag=None, with_noise=True):
        D, mus = self.D, self.mus
        sigmas = np.exp(self._log_sigmasq) if with_noise else np.zeros(
            (self.K, self.D))
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        for k in range(self.K):
            ...
            self._log_sigmasq[k] = np.log(
                np.average(sqerr, weights=weights[:, k], axis=0))
...
class StudentsTObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.independent_studentst_logpdf(data[:, None, :], mus,
                                                  sigmas, nus,
                                                  mask=mask[:, None, :])
    ...
    def m_step(self, expectations, datas, inputs, masks, tags, **kwargs):
        ...
        parameters using the EM algorithm. See the notebook in
        doc/students_t for
        complete details.
        """
    ...
    def _m_step_mu_sigma(self, expectations, datas, inputs, masks, tags):
        ...
        E_taus = []
        for y in datas:
            # nu: (K,D)  mus: (K, D)  sigmas: (K, D)  y: (T, D)  -> tau: (T,
            # K, D)
            alpha = self.nus / 2 + 1 / 2
            beta = self.nus / 2 + 1 / 2 * (
                        y[:, None, :] - self.mus) ** 2 / self.sigmasq
        ...
        for E_tau, (Ez, _, _), y in zip(E_taus, expectations, datas):
            sqerr += np.sum(
                Ez[:, :, None] * E_tau * (y[:, None, :] - self.mus) ** 2,
                axis=0)
    ...
    def _m_step_nu(self, expectations, datas, inputs, masks, tags):
        ...
        for y, (Ez, _, _) in zip(datas, expectations):
            # nu: (K,D)  mus: (K, D)  sigmas: (K, D)  y: (T, D)  ->
            # alpha/beta: (T, K, D)
            alpha = self.nus / 2 + 1 / 2
            beta = self.nus / 2 + 1 / 2 * (
                        y[:, None, :] - self.mus) ** 2 / self.sigmasq
            ...
            E_logtaus += np.sum(
                Ez[:, :, None] * (digamma(alpha) - np.log(beta)), axis=0)
        ...
        for k in range(K):
            for d in range(D):
                self._log_nus[k, d] = np.log(
                    generalized_newton_studentst_dof(E_taus[k, d],
                                                     E_logtaus[k, d]))
...
class MultivariateStudentsTObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        assert np.all(
            mask), "MultivariateStudentsTObservations does not support " \
                   "missing data"
        ...
        # arrays as inputs
        return np.column_stack(
            [stats.multivariate_studentst_logpdf(data, mu, Sigma, nu)
             for mu, Sigma, nu in zip(mus, Sigmas, nus)])

        # return stats.multivariate_studentst_logpdf(data[:, None, :], mus,
        # Sigmas, nus)
    ...
    def _m_step_mu_sigma(self, expectations, datas, inputs, masks, tags):
        ...
        E_taus = []
        for y in datas:
            # nu: (K,)  mus: (K, D)  Sigmas: (K, D, D)  y: (T, D)  -> tau: (
            # T, K)
            alpha = self.nus / 2 + D / 2
            beta = self.nus / 2 + 1 / 2 * stats.batch_mahalanobis(
                self._sqrt_Sigmas, y[:, None, :] - self.mus)
            E_taus.append(alpha / beta)
        ...
        for E_tau, (Ez, _, _), y in zip(E_taus, expectations, datas):
            ...
            h += np.sum(Ez[:, :, None] * E_tau[:, :, None] * y[:, None, :],
                        axis=0)
        self.mus = h / J[:, None]
        ...
        weight = np.zeros((K,))
        for E_tau, (Ez, _, _), y in zip(E_taus, expectations, datas):
            # sqerr += np.sum(Ez[:, :, None] * E_tau * (y[:, None,
            # :] - self.mus)**2, axis=0)
            resid = y[:, None, :] - self.mus
        ...
        self._sqrt_Sigmas = np.linalg.cholesky(
            sqerr / weight[:, None, None] + 1e-8 * np.eye(D))
    ...
    def _m_step_nu(self, expectations, datas, inputs, masks, tags):
        ...
        weights = np.zeros(K)
        for y, (Ez, _, _) in zip(datas, expectations):
            # nu: (K,)  mus: (K, D)  Sigmas: (K, D, D)  y: (T, D)  ->
            # alpha/beta: (T, K)
            alpha = self.nus / 2 + D / 2
            beta = self.nus / 2 + 1 / 2 * stats.batch_mahalanobis(
                self._sqrt_Sigmas, y[:, None, :] - self.mus)
...
class BernoulliObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.bernoulli_logpdf(data[:, None, :], self.logit_ps,
                                      mask=mask[:, None, :])
...
class PoissonObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        lambdas = np.exp(self.log_lambdas)
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.poisson_logpdf(data[:, None, :], lambdas,
                                    mask=mask[:, None, :])
...
class CategoricalObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.categorical_logpdf(data[:, None, :], self.logits,
                                        mask=mask[:, None, :])

    def sample_x(self, z, xhist, input=None, tag=None, with_noise=True):
        ps = np.exp(
            self.logits - logsumexp(self.logits, axis=2, keepdims=True))
        return np.array(
...
class InputDrivenObservations(Observations):
    ...
    def __init__(self, K, D, M=0, C=2, prior_mean=0, prior_sigma=1000):
        ...
        @param prior_sigma: parameter governing strength of prior. Prior on
        GLM weights is multivariate
        normal distribution with mean 'prior_mean' and diagonal covariance
        matrix (prior_sigma is on diagonal)
        """
    ...
    def calculate_logits(self, input):
        ...
        :return: array of size TxKxC containing log(pr(yt=c|zt=k, ut)) for
        all c in {1, ..., C} and k in {1, ..., K}
        """
        # Transpose array dimensions, so that array is now of shape ((
        # C-1)xKx(M+1))
        Wk_tranpose = np.transpose(self.Wk, (1, 0, 2))
        # Stack column of zeros to transform array from size ((C-1)xKx(M+1))
        # to ((C)xKx(M+1)) and then transform shape back to (KxCx(M+1))
        Wk = np.transpose(np.vstack([Wk_tranpose, np.zeros(
            (1, Wk_tranpose.shape[1], Wk_tranpose.shape[2]))]),
                          (1, 0, 2))
        # Input effect; transpose so that output has dims TxKxC
        time_dependent_logits = np.transpose(np.dot(Wk, input.T), (2, 0,
                                                                   1))  #
        # Note: this has an unexpected effect when both input (and thus Wk)
        # are empty arrays and returns an array of zeros
    ...
    def sample_x(self, z, xhist, input=None, tag=None, with_noise=True):
        ...
        self.M,):  # if input is vector of size self.M (one time point),
            # expand dims to be (1, M)
        ...
        if T == 1:
            sample = np.array(
                [npr.choice(self.C, p=ps[t, z]) for t in range(T)])
        elif T > 1:
            sample = np.array(
                [npr.choice(self.C, p=ps[t, z[t]]) for t in range(T)])
        return sample
    ...
    def _compute_mus(self, data, input, mask, tag):
        ...
        for k, (A, b, V, mu0) in enumerate(zip(As, bs, Vs, mu0s)):
            ...
            for l in range(self.lags):
                Al = A[:, l * D:(l + 1) * D]
                mus_k_ar = mus_k_ar + np.dot(data[self.lags - l - 1:-l - 1],
                                             Al.T)
            mus_k_ar = mus_k_ar + b
...
class AutoRegressiveObservations(_AutoRegressiveObservationsBase):
    ...
    def __init__(self, K, D, M=0, lags=1,
        ...
        self._As = .80 * np.array([
            np.column_stack(
                [random_rotation(D), np.zeros((D, (lags - 1) * D))])
            for _ in range(K)])
    ...
    def Sigmas_init(self):
        return np.matmul(self._sqrt_Sigmas_init,
                         np.swapaxes(self._sqrt_Sigmas_init, -1, -2))

    @Sigmas_init.setter
    def Sigmas_init(self, value):
        assert value.shape == (self.K, self.D, self.D)
        self._sqrt_Sigmas_init = np.linalg.cholesky(
            value + 1e-8 * np.eye(self.D))
    ...
    @property
    def params(self):
        return super(AutoRegressiveObservations, self).params + (
        self._sqrt_Sigmas,)

    @params.setter
    def params(self, value):
        self._sqrt_Sigmas = value[-1]
        super(AutoRegressiveObservations, self.__class__).params.fset(self,
                                                                      value[
                                                                      :-1])
    ...
    def log_likelihoods(self, data, input, mask, tag=None):
        assert np.all(
            mask), "Cannot compute likelihood of autoregressive obsevations " \
                   "with missing data."
        L = self.lags
        ...
        # (D,D), and (,)
        # arrays as inputs
        ll_init = np.column_stack(
            [stats.multivariate_normal_logpdf(data[:L], mu[:L], Sigma)
             for mu, Sigma in zip(mus, self.Sigmas_init)])

        ll_ar = np.column_stack(
            [stats.multivariate_normal_logpdf(data[L:], mu[L:], Sigma)
    ...
    def _get_sufficient_statistics(self, expectations, datas, inputs):
            ...
            for k in range(K):
                ...
                for l1 in range(lags):
                    ...
                    for l2 in range(l1, lags):
                        ...
                        ExuxuTs[k, l1 * D:(l1 + 1) * D,
                        l2 * D:(l2 + 1) * D] += np.einsum('t,ti,tj->ij', w, x1,
                                                          x2)

                    # Cross terms between lagged data and inputs and bias
                    ExuxuTs[k, l1 * D:(l1 + 1) * D,
                    D * lags:D * lags + M] += np.einsum('t,ti,tj->ij', w, x1,
                                                        u)
                    ExuxuTs[k, l1 * D:(l1 + 1) * D, -1] += np.einsum('t,ti->i',
                                                                     w, x1)

                ExuxuTs[k, D * lags:D * lags + M,
                D * lags:D * lags + M] += np.einsum('t,ti,tj->ij', w, u, u)
                ExuxuTs[k, D * lags:D * lags + M, -1] += np.einsum('t,ti->i',
                                                                   w, u)
                ...
                for l1 in range(lags):
                    x1 = data[lags - l1 - 1:-l1 - 1]
                    ExuyTs[k, l1 * D:(l1 + 1) * D, :] += np.einsum(
                        't,ti,tj->ij', w, x1, y)
                ExuyTs[k, D * lags:D * lags + M, :] += np.einsum('t,ti,tj->ij',
                                                                 w, u, y)
        ...
        for k in range(K):
            for l1 in range(lags):
                for l2 in range(l1, lags):
                    ExuxuTs[k, l2 * D:(l2 + 1) * D,
                    l1 * D:(l1 + 1) * D] = ExuxuTs[k, l1 * D:(l1 + 1) * D,
                                           l2 * D:(l2 + 1) * D].T
                ExuxuTs[k, D * lags:D * lags + M,
                l1 * D:(l1 + 1) * D] = ExuxuTs[k, l1 * D:(l1 + 1) * D,
                                       D * lags:D * lags + M].T
                ExuxuTs[k, -1, l1 * D:(l1 + 1) * D] = ExuxuTs[k,
                                                      l1 * D:(l1 + 1) * D,
                                                      -1].T
            ExuxuTs[k, -1, D * lags:D * lags + M] = ExuxuTs[k,
                                                    D * lags:D * lags + M,
                                                    -1].T

        return ExuxuTs, ExuyTs, EyyTs, Ens

    def _extend_given_sufficient_statistics(self, expectations,
                                            continuous_expectations, inputs):
        # Extend continuous_expectations with given inputs and discrete weights
        assert self.lags == 1, "_extend_given_sufficient_statistics assumes " \
                               "lags == 1."
        K, D, M, lags = self.K, self.D, self.M, self.lags
        D_in = D * lags + M + 1
        ...
        for (Ez, _, _), (_, Ex, smoothed_sigmas, Exxn), u in \
            ...
            for k in range(K):
                ...
                ExuxuTs[k, :D, D:D + M] += np.einsum('t,ti,tj->ij', w, Ex[:-1],
                                                     u)
                ExuxuTs[k, :D, -1] += np.einsum('t,ti->i', w, Ex[:-1])
                ExuxuTs[k, D:D + M, D:D + M] += np.einsum('t,ti,tj->ij', w, u,
                                                          u)
    ...
    def m_step(self, expectations, datas, inputs, masks, tags,
        ...
        compute an exact M-step using the expected sufficient statistics for
        the
        continuous states. In this case, we ignore the prior provided by (
        J0, h0),
        because the calculation is exact. `continuous_expectations` should
        be a tuple of
        (Ex, Ey, ExxT, ExyT, EyyT).

        If `continuous_expectations` is None, we use `datas` and `expectations,
        and (optionally) the prior given by (J0, h0). In this case,
        we estimate the sufficient
        statistics using `datas,` which is typically a single sample of the
        continuous
        ...
        for k in range(K):
            Wk = np.linalg.solve(ExuxuTs[k] + self.J0[k],
                                 ExuyTs[k] + self.h0[k]).T
            As[k] = Wk[:, :D * lags]
            Vs[k] = Wk[:, D * lags:-1]
            bs[k] = Wk[:, -1]
        ...
        # If any states are unused, set their parameters to a perturbation
        # of a used state
    ...
    def neg_hessian_expected_log_dynamics_prob(self, Ez, data, input, mask,
                                               tag=None):
        assert np.all(
            mask), "Cannot compute negative Hessian of autoregressive " \
                   "obsevations with missing data."
        assert self.lags == 1, "Does not compute negative Hessian of " \
                               "autoregressive observations with lags > 1"

        # initial distribution contributes a Gaussian term to first diagonal
        # block
        J_ini = np.sum(Ez[0, :, None, None] * np.linalg.inv(self.Sigmas_init),
                       axis=0)

        # first part is transition dynamics - goes to all terms except final
        # one
        # E_q(z) x_{t} A_{z_t+1}.T Sigma_{z_t+1}^{-1} A_{z_t+1} x_{t}
        inv_Sigmas = np.linalg.inv(self.Sigmas)
        dynamics_terms = np.array([A.T @ inv_Sigma @ A for A, inv_Sigma in
                                   zip(self.As,
                                       inv_Sigmas)])  # A^T Qinv A terms
        J_dyn_11 = np.sum(Ez[1:, :, None, None] * dynamics_terms[None, :],
                          axis=1)

        # second part of diagonal blocks are inverse covariance matrices -
        # goes to all but first time bin
        ...
        off_diag_terms = np.array(
            [inv_Sigma @ A for A, inv_Sigma in zip(self.As, inv_Sigmas)])
        J_dyn_21 = -1 * np.sum(Ez[1:, :, None, None] * off_diag_terms[None, :],
                               axis=1)
...
class AutoRegressiveDiagonalNoiseObservations(AutoRegressiveObservations):
    ...
    def __init__(self, K, D, M=0, lags=1,
        ...
        # Get rid of the square root parameterization and replace with log
        # diagonal
    ...
    def Sigmas_init(self):
        return np.array(
            [np.diag(np.exp(log_s)) for log_s in self._log_sigmasq_init])

    @property
    def Sigmas(self):
        return np.array(
            [np.diag(np.exp(log_s)) for log_s in self._log_sigmasq])
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        # arrays as inputs
        ll_init = np.column_stack(
            [stats.diagonal_gaussian_logpdf(data[:L], mu[:L], sigmasq)
...
class IndependentAutoRegressiveObservations(_AutoRegressiveObservationsBase):
    def __init__(self, K, D, M=0, lags=1):
        ...
        self._As = np.concatenate(
            (.95 * np.ones((K, D, 1)), np.zeros((K, D, lags - 1))), axis=2)
        self._log_sigmasq_init = np.zeros((K, D))
        self._log_sigmasq = np.zeros((K, D))
    ...
    def _compute_mus(self, data, input, mask, tag):
        ...
        mus = np.matmul(Vs[None, ...], input[self.lags:, None, :self.M, None])[
              :, :, :, 0]
        for l in range(self.lags):
        ...
        mus = np.concatenate(
            (self.mu_init * np.ones((self.lags, self.K, self.D)), mus))
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        # Compute the likelihood of the initial data and remainder separately
        L = self.lags
        ll_init = stats.diagonal_gaussian_logpdf(data[:L, None, :], mus[:L],
                                                 self.sigmasq_init)
        ll_ar = stats.diagonal_gaussian_logpdf(data[L:, None, :], mus[L:],
                                               self.sigmasq)
        return np.row_stack((ll_init, ll_ar))
        ...
        for d in range(self.D):
            ...
            for (Ez, _, _), data, input, mask in zip(expectations, datas,
                ...
                if np.all(mask[:, d]):
                    xs.append(
                        np.hstack(
                            [data[self.lags - l - 1:-l - 1, d:d + 1] for l in
                             range(self.lags)]
                            + [input[self.lags:, :M],
            ...
            # Otherwise, fit a weighted linear regression for each discrete
            # state
            for k in range(self.K):
                ...
                Jk = np.sum(
                    weights[:, k][:, None, None] * xs[:, :, None] * xs[:, None,
                                                                    :], axis=0)
...
class _RobustAutoRegressiveObservationsMixin(object):
    ...
    This mixin mus be used in conjunction with either
    AutoRegressiveObservations or
    AutoRegressiveDiagonalNoiseObservations, which provides the
    parameterization for
    Sigma.  The mixin does not capitalize on structure in Sigma, so it will pay
    a small complexity penalty when used in conjunction with the diagonal
    noise model.
    """
    ...
    @property
    def params(self):
        return super(_RobustAutoRegressiveObservationsMixin, self).params + (
        self._log_nus,)

    @params.setter
    def params(self, value):
        self._log_nus = value[-1]
        super(_RobustAutoRegressiveObservationsMixin,
              self.__class__).params.fset(self, value[:-1])
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        ll_ar = np.column_stack(
            [stats.multivariate_studentst_logpdf(data[L:], mu[L:], Sigma, nu)
        ...
        for (Ez, _, _), data, input, mask, tag in zip(expectations, datas,
            ...
            xs.append(
                np.hstack([data[lags - l - 1:-l - 1] for l in range(lags)]
                          + [input[lags:, :self.M],
                             np.ones((data.shape[0] - lags, 1))]))
            ys.append(data[lags:])
            Ezs.append(Ez[lags:])

        for itr in range(num_em_iters):
            # E Step: compute expected precision for each data point given
            # current parameters
            taus = []
            for x, y in zip(xs, ys):
                Afull = np.concatenate((self.As, self.Vs, self.bs[:, :, None]),
                                       axis=2)
                mus = np.matmul(Afull[None, :, :, :], x[:, None, :, None])[:,
                      :, :, 0]

                # nu: (K,)  mus: (T, K, D)  sigmas: (K, D, D)  y: (T,
                # D)  -> tau: (T, K)
                alpha = self.nus / 2 + D / 2
                sqrt_Sigmas = np.linalg.cholesky(self.Sigmas)
                beta = self.nus / 2 + 1 / 2 * stats.batch_mahalanobis(
                    sqrt_Sigmas, y[:, None, :] - mus)
                taus.append(alpha / beta)

            # M step: Fit the weighted linear regressions for each K and D
            # This is exactly the same as the M-step for the
    ...
    def _m_step_nu(self, expectations, datas, inputs, masks, tags):
        """
        Update the degrees of freedom parameter of the multivariate t
        distribution
        ...
        for (Ez, _, _,), data, input, mask, tag in zip(expectations, datas,
                                                       inputs, masks, tags):
            # nu: (K,)  mus: (T, K, D)  Sigmas: (K, D, D)  y: (T, D)  ->
            # tau: (T, K)
            mus = np.swapaxes(self._compute_mus(data, input, mask, tag), 0, 1)

            alpha = self.nus / 2 + D / 2
            sqrt_Sigma = np.linalg.cholesky(self.Sigmas)
            # TODO: Performance could be improved by iterating over K
            #  outside batch_mahalanobis
            beta = self.nus / 2 + 1 / 2 * stats.batch_mahalanobis(sqrt_Sigma,
                                                                  data[L:,
                                                                  None,
                                                                  :] - mus[L:])

            E_taus += np.sum(Ez[L:, :] * alpha / beta, axis=0)
            E_logtaus += np.sum(Ez[L:, :] * (digamma(alpha) - np.log(beta)),
                                axis=0)
            weights += np.sum(Ez, axis=0)
    ...
    def sample_x(self, z, xhist, input=None, tag=None, with_noise=True):
        D, As, bs, Vs, Sigmas, nus = self.D, self.As, self.bs, self.Vs, \
                                     self.Sigmas, self.nus
        if xhist.shape[0] < self.lags:
            S = np.linalg.cholesky(self.Sigmas_init[z]) if with_noise else 0
            return self.mu_init[z] + np.dot(S, npr.randn(D))
        else:
...
class RobustAutoRegressiveObservations(_RobustAutoRegressiveObservationsMixin,
                                       AutoRegressiveObservations):
    """
    AR model where the noise is distributed according to a multivariate t
    distribution,
...
class RobustAutoRegressiveObservationsNoInput(
...
class RobustAutoRegressiveDiagonalNoiseObservations(
    _RobustAutoRegressiveObservationsMixin,
...
class AltRobustAutoRegressiveDiagonalNoiseObservations(
    ...
    def __init__(self, K, D, M=0, lags=1):
        super(AltRobustAutoRegressiveDiagonalNoiseObservations, self).__init__(
            K, D, M=M, lags=lags)
        self._log_nus = np.log(4) * np.ones((K, D))
    ...
    def permute(self, perm):
        super(AltRobustAutoRegressiveDiagonalNoiseObservations, self).permute(
            perm)
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        ll_init = stats.diagonal_gaussian_logpdf(data[:L, None, :], mus[:L],
                                                 self.sigmasq_init)
        ll_ar = stats.independent_studentst_logpdf(data[L:, None, :], mus[L:],
                                                   self.sigmasq, self.nus)
        return np.row_stack((ll_init, ll_ar))
    ...
    def m_step(self, expectations, datas, inputs, masks, tags,
        ...
        """
        self._m_step_ar(expectations, datas, inputs, masks, tags, num_em_iters)
        self._m_step_nu(expectations, datas, inputs, masks, tags, optimizer,
                        num_iters, **kwargs)
    ...
    def _m_step_ar(self, expectations, datas, inputs, masks, tags,
        ...
        for itr in range(num_em_iters):
            ...
            for x, y in zip(xs, ys):
                ...
                alpha = self.nus / 2 + 1 / 2
                beta = self.nus / 2 + 1 / 2 * (
                            y[:, None, :] - mus) ** 2 / self.sigmasq
                taus.append(alpha / beta)

            # M step: Fit the weighted linear regressions for each K and D
            J = np.tile(np.eye(D * lags + M + 1)[None, None, :, :],
                        (K, D, 1, 1))
    ...
    def _m_step_nu(self, expectations, datas, inputs, masks, tags, optimizer,
        ...
        for (Ez, _, _,), data, input, mask, tag in zip(expectations, datas,
            ...
            alpha = self.nus / 2 + 1 / 2
            beta = self.nus / 2 + 1 / 2 * (
                        data[L:, None, :] - mus[L:]) ** 2 / self.sigmasq

            E_taus += np.sum(Ez[L:, :, None] * alpha / beta, axis=0)
            E_logtaus += np.sum(
                Ez[L:, :, None] * (digamma(alpha) - np.log(beta)), axis=0)
            weights += np.sum(Ez, axis=0)
...
class VonMisesObservations(Observations):
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        mask = np.ones_like(data, dtype=bool) if mask is None else mask
        return stats.vonmises_logpdf(data[:, None, :], mus, kappas,
                                     mask=mask[:, None, :])
