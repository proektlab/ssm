...
gen_weights = np.array([[[4, 0, 0, 0, 0, 0]], [[2, 1, 1, 0, 0, 0]]])
'''
np.array([[[1/((2/13)+np.exp(2-(t/8))), 1/((2/13)+np.exp(4-(t/6))), 1/((2/13)+np.exp(6-(t/5))), -0.2, 0.2, 1/((1/4)+np.exp((t/10)-4))]],
                        ...
                        [[1/((1/2)+np.exp(2-(t/8))), 1/((1/2)+np.exp(4-(t/6))), 1/((1/2)+np.exp(6-(t/5))), -2.5, 1, 0.2]]])
                        '''
gen_log_trans_mat = np.log(np.array([[[0.95, 0.05], [0.05, 0.95]]]))
...
num_sess = 5 # number of example sessions
num_trials_per_sess = 240 # number of trials in a session
...
gtOutputs = [1, 2]
combinations = np.array(list(itertools.product(wrongCueNums, gtOutputs))) # incorrect cue and correct direction combinations (12)
...
print(wrongCueNum)
'''
num_trials_per_sess -= 1
wrongCueNum = np.delete(wrongCueNum, [num_trials_per_sess, num_trials_per_sess - 1, num_trials_per_sess - 2, num_trials_per_sess - 3,
                                      num_trials_per_sess - 4, num_trials_per_sess - 5, num_trials_per_sess - 6, num_trials_per_sess - 7,
                                      num_trials_per_sess - 8, num_trials_per_sess - 9, num_trials_per_sess - 10, num_trials_per_sess - 11])
gtOutput = np.delete(gtOutput, [num_trials_per_sess, num_trials_per_sess - 1, num_trials_per_sess - 2, num_trials_per_sess - 3,
                                      num_trials_per_sess - 4, num_trials_per_sess - 5, num_trials_per_sess - 6, num_trials_per_sess - 7,
                                      num_trials_per_sess - 8, num_trials_per_sess - 9, num_trials_per_sess - 10, num_trials_per_sess - 11])

print(wrongCueNum.shape)
num_trials_per_sess -= 11
...
'''
    ...
    inpts[:,:,2] = np.random.choice(stim_vals, (num_sess, num_trials_per_sess))
    inpts[:,0,5] = np.random.choice(prev_correct_vals, num_sess)
'''
...
for k in range(num_states):
    for i in range(input_dim):
        numerator[k] += (gen_weights[k][0][i] - gen_weights_mean[k]) * (recovered_weights[k][0][i] - rec_weights_mean[k])
