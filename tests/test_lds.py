...
from ssm_customized.util import SEED
import copy
import scipy
...
from ssm_customized.primitives import \
...
def test_lds_sample_and_fit(T=100, N=15, K=3, D=10, num_cases=25):
    ...
    for idx in test_case_indices:
        ...
        for posterior in POSTERIORS[method]:
            ...
            true_slds = ssm_customized.SLDS(N, K, D,
                                transitions=transitions,
                                dynamics=dynamics,
                                emissions=emissions)
            z, x, y = true_slds.sample(T)

            fit_slds = ssm_customized.SLDS(N, K, D,
                                transitions=transitions,
                                dynamics=dynamics,
                                emissions=emissions)
            fit_slds.fit(y,
...
def test_laplace_em(T=100, N=15, K=3, D=10, num_cases=25):
    ...
    for idx in test_case_indices:
        ...
        true_slds = ssm_customized.SLDS(N, K, D, M=input_dim,
                                transitions=transitions,
                                dynamics="gaussian",
                                emissions=emissions)
        ...
        fit_slds = ssm_customized.SLDS(N, K, D, M=input_dim,
                            transitions=transitions,
                            dynamics="gaussian",
                            emissions=emissions)
        try:
...
def test_laplace_em_hessian(N=5, K=3, D=2, T=20):
    for transitions in ["standard", "recurrent", "recurrent_only"]:
        for emissions in ["gaussian_orthog", "gaussian"]:
            ...
            slds = ssm_customized.SLDS(N, K, D, transitions=transitions,
                            dynamics="gaussian",
                            emissions=emissions)
            z, x, y = slds.sample(T)
            new_slds = ssm_customized.SLDS(N, K, D, transitions="standard",
                            dynamics="gaussian",
                            emissions=emissions)
