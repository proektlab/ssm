...
from ssm_customized.primitives import lds_log_probability
...
def time_laplace_em_end2end(ncalls=5):
    ...
    for (N, D, T, num_datas) in params_list:
        lds_true = ssm_customized.LDS(N, D, dynamics="gaussian", emissions="gaussian")
        datas = [lds_true.sample(T)[1] for _ in range(num_datas)]

        lds_new = ssm_customized.LDS(N, D, dynamics="gaussian", emissions="gaussian")
