...
def test_sample(T=10, K=4, D=3, M=2):
    ...
    for transitions in transition_names:
        for observations in observation_names:
            hmm = ssm_customized.HMM(K, D, M=0, transitions=transitions, observations=observations)
    ...
    for transitions in transition_names:
        for observations in observation_names:
            hmm = ssm_customized.HMM(K, D, M=M, transitions=transitions, observations=observations)
...
def test_constrained_hmm(T=100, K=3, D=3):
    hmm = ssm_customized.HMM(K, D, M=0,
    ...
    fit_hmm = ssm_customized.HMM(K, D, M=0,
                  transitions="constrained",
                  observations="gaussian",
...
def test_hmm_likelihood(T=1000, K=5, D=2):
    ...
    hmm = ssm_customized.HMM(K, D, observations="diagonal_gaussian")
...
def test_hmm_mp_perf(T=10000, K=100, D=20):
    ...
    from ssm_customized.messages import forward_pass, backward_pass
...
def test_hmm_likelihood_perf(T=10000, K=50, D=20):
    ...
    # Make an HMM with these parameters
    hmm = ssm_customized.HMM(K, D, observations="gaussian")
    hmm.transitions.log_Ps = np.log(A)
    hmm.observations.mus = C
    ...
    arhmm = ssm_customized.HMM(K, D, observations="ar")
    tic = time()
...
def test_trace_product():
    ...
    assert np.allclose(ssm_customized.util.trace_product(A, B),
...
def test_SLDSStructuredMeanField_entropy():
    ...
    info_args = ssm_customized.messages.convert_mean_to_info_args(*params)
    ...
    # J_obs[1:] += J_dyn_22
    # J_dyn_22[:] = 0
    log_Z, smoothed_mus, smoothed_Sigmas, ExxnT = ssm_customized.messages.\
        kalman_info_smoother(*info_args)


    # Model is just a dummy model to simplify 
    # instantiating the posterior object.
    model = ssm_customized.SLDS(N, 1, D, emissions="gaussian", dynamics="gaussian")
    datas = params[-1]
    post = ssm_customized.variational.SLDSStructuredMeanFieldVariationalPosterior(model, datas)
